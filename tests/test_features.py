...
import librosax as librosa

# librosax does not implement I/O - srand helper for remaining tests
def srand(seed=628318530):
    np.random.seed(seed)

# Skip marker for tests requiring I/O functions not in librosax
requires_io = pytest.mark.skip(reason="librosax does not implement audio I/O - use librosa for that")
...
# Removed: y_ex fixture - librosax does not implement load()
# Tests using this fixture are marked with @requires_io
...
def test_fourier_tempogram_invert(sr, hop_length, win_length, center, window):
    if not center:
        pytest.xfail("istft center=False is not implemented")
...
# Removed: tg_ex fixture - depends on y_ex which requires load()
